...
class FolderController extends Controller
{
  ...
  public function traitement(Request $request)
  {
    ...
    if ((!empty($request->all())) && auth()->user()->apply->folder_id == NULL) { // test si le folder n'existe pas (==null) cad qu'il n'a pas encore été créer
      ...
      if (!empty($request->hasFile('cv'))) {
        $path = $request->cv->storeAs('storage', auth()->user()->id . '_' . $request->cv->getClientOriginalName());
        $monDossier->cv = str_replace('storage/', '', $path);
      }
      if (!empty($request->hasFile('screenshot'))) {
        $path = $request->screenshot->storeAs('storage', auth()->user()->id . '_' . $request->screenshot->getClientOriginalName());
        $monDossier->screenshot = str_replace('storage/', '', $path);
      }
      if (!empty($request->hasFile('coverletter'))) {
        $path = $request->coverletter->storeAs('storage', auth()->user()->id . '_' . $request->coverletter->getClientOriginalName());
        $monDossier->coverletter = str_replace('storage/', '', $path);
      }
      if (!empty($request->hasFile('bulletin'))) {
        $path = $request->bulletin->storeAs('storage', auth()->user()->id . '_' . $request->bulletin->getClientOriginalName());
        $monDossier->bulletin = str_replace('storage/', '', $path);
      }
      if (!empty($request->hasFile('registrationForm'))) {
        $path = $request->bulletin->storeAs('storage', auth()->user()->id . '_' . $request->registrationForm->getClientOriginalName());
      ...
      if (strcmp($libelle, $incomplet) == 0) {
        ...
        $monDossier = FolderService::update($request->id, $request->cv, $request->coverletter, $request->screenshot, $request->bulletin);
      } else {
